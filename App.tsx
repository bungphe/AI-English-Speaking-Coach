...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Load keys from window object injected by entrypoint
    const keys = (window as any).API_KEYS;
    if (keys && Array.isArray(keys) && keys.length > 0) {
      setApiKeys(keys);
      setIsLoadingKeys(false);
    } else {
      console.error("No API keys found in window.API_KEYS");
      setStatusText("Configuration error: No API keys found.");
      setIsLoadingKeys(false);
    }
  }, []);

  // Memoize the AI client to recreate it only when the key index changes
  const ai = React.useMemo(() => {
    if (apiKeys.length === 0) return null;
    const key = apiKeys[currentKeyIndex % apiKeys.length];
    console.log(`Using API Key index: ${currentKeyIndex}`);
    return new GoogleGenAI({ apiKey: key });
  }, [currentKeyIndex, apiKeys]);
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      if (!ai) {
        setStatusText("API Keys not loaded.");
        return;
      }
      ...
      if (error.message?.includes('429') || error.toString().includes('Quota exceeded') || error.toString().includes('429')) {
        console.log("Quota exceeded, rotating key...");
        setCurrentKeyIndex(prev => prev + 1);
        setStatusText("Quota limit reached. Switching API key and retrying...");
        setTimeout(() => {
      ...
      setStatusText("Could not access camera/mic or connection failed. Check permissions/network.");
      setIsSessionActive(false);
    }
  }, [ai, stopSession, currentAvatar.name, practiceMode, isSessionActive]);
  ...
  const generateFeedback = async (finalTranscript: TranscriptEntry[]) => {
    ...
    try {
      ...
      if (error.message?.includes('429') || error.toString().includes('Quota exceeded')) {
        console.log("Quota exceeded during feedback, rotating key...");
        setCurrentKeyIndex(prev => prev + 1);
        // Retry once with new key? 
        // For simplicity, just return a message, or we could implement a retry loop.
        return "Quota limit reached during feedback generation. Please try again.";
      }
